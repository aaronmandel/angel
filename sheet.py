...
def get_task_sheet():
    try:
        ...
        sheet = client.open("YourSpreadsheetName")
        task_sheet = sheet.add_worksheet(title=TASK_SHEET, rows="1000", cols="5")
...
def delete_task(user_id: str, task_name: str) -> bool:
    ...
    for i, row in enumerate(rows, start=2):  # skip header row
        if row["user_id"] == str(user_id) and row["name"] == task_name:
            sheet.delete_rows(i)
            return True
...
def mark_task_complete(user_id: str, name: str):
    ...
    for i, row in enumerate(rows, start=2):
        if row["user_id"] == str(user_id) and row["name"] == name:
...
def edit_task(user_id: str, original_name: str, new_name: str = None, new_due_date: str = None):
    ...
    for i, row in enumerate(rows, start=2):
        if row["user_id"] == str(user_id) and row["name"] == original_name:
            if new_name:
