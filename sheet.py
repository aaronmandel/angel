...
# Load spreadsheet ID (more reliable than name)
...
def get_timezone_sheet():
    try:
        return client.open_by_key(SPREADSHEET_ID).worksheet(TIMEZONE_SHEET)
...
def get_task_sheet():
    try:
        return client.open_by_key(SPREADSHEET_ID).worksheet(TASK_SHEET)
    except gspread.exceptions.WorksheetNotFound:
        sheet = client.open_by_key(SPREADSHEET_ID)
